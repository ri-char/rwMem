...
void CtestHwBpClientDlg::OnBnClickedButtonAddHwbp() {
	...
	for (USER_HIT_INFO h : vHitResult) {
		...
		wchar_t info[4096] = { 0 };
		wsprintf(info, L"命中地址: 0x%Ix 命中次数:%u \r\n\r\n", h.hit_addr, h.hit_count);
		ssInfoText << info;

		for (int r = 0; r < 30; r += 5) {
			memset(info, 0, sizeof(info));
			wsprintf(info, L"X%-2d=%-12Ix X%-2d=%-12Ix X%-2d=%-12Ix X%-2d=%-12Ix X%-2d=%-12Ix\r\n",
				r, h.regs.regs[r],
				r + 1, h.regs.regs[r + 1],
				r + 2, h.regs.regs[r + 2],
				r + 3, h.regs.regs[r + 3],
		...
		memset(info, 0, sizeof(info));
		wsprintf(info, L"process status: %-12Ix orig_x0: %-12Ix\r\nsyscallno: %-12Ix\r\n",
			h.regs.pstate,
			h.regs.orig_x0,
			h.regs.syscallno);
